...
## v0.12.3 (2026-08-29) — 폰 동기화가 조용히 죽던 문제

**증상**: 폰에서 받은 문자가 웹에 안 뜸. **원인**: JWT 7일 만료. 폰 기기 등록이 08-21 13:24, 마지막 접속이 08-28 13:17 — 만료 7분 전이 마지막이었다. 웹은 만료되면 로그인 화면이 떠서 비밀번호를 다시 넣게 되지만, 폰 브리지는 인증 거부를 받으면 **로그만 남기고 세션을 지운 뒤 조용히 종료**했다. 아무도 모르는 채 하루 동안 동기화가 죽어 있었다.

- **서버**: `POST /api/token-refresh` — 유효한 토큰이 새 7일 토큰으로 교체(슬라이딩 갱신). 유효한 토큰이 이미 할 수 있는 것 이상을 주지 않으며, 로그아웃·폐기로 세션 버전이 돌면 갱신도 즉시 거부된다(회귀 테스트 포함).
- **Android**: 브리지가 릴레이 접속 성공 시 6시간에 한 번 토큰을 갱신해 Keystore에 저장 — 일주일에 한 번이라도 문자가 오면 세션이 영원히 이어진다. 그리고 **인증 거부 시 알림을 띄운다**("문자 동기화가 중단되었습니다 — 다시 로그인하세요"). 다시는 조용히 죽지 않는다.
- **웹**: 로그인된 앱 로드마다 토큰 갱신.
- 부수 수정: `Credentials.updateToken` 추가(기존 `saveSecretsUnlocked`는 clearSession 전용으로 토큰을 항상 비워 저장하는 함수라 재사용 불가).

- Android versionCode 26 / versionName 0.12.3. 서버 80 · 웹 153 · Android 200 테스트 통과.

## v0.12.2 (2026-08-27) — 이슈 #5·#6

### #5 "시스템 설치 기다리는중" (수정)

- **버튼 없는 대기 배너가 탈출구 없는 흡수 상태였다.** 설치 세션을 넘기기 전에 `SESSION_SUBMITTED`를 저장하는데, 설치가 성공하면 프로세스가 교체돼 결과 콜백이 죽은 프로세스로 간다. 실패·취소 쪽도 콜백이 유실되면 같은 상태에 갇히고, 이 상태는 자동·수동 업데이트 확인까지 전부 막았다.
- 이제 콜드 스타트에서 `SESSION_SUBMITTED`를 발견하면: 설치가 실제로 반영됐으면 정리하고, 아니면 **재시도·닫기가 있는 실패 배너로 전환**한다. 제출한 프로세스가 죽은 시점에 "기다리는 중"은 성립하지 않는다.
- 폴백(레거시 설치 화면) 경로도 성공 시 정리된다 — 이전에는 설치가 됐는데도 다음 실행에서 "완료되지 않았습니다"를 띄웠다.
- recents로 돌아오는 경우를 위해 onResume에서도 성공 판정만 수행한다(설치 화면이 떠 있는 동안의 경쟁을 피하려고 실패 전환은 콜드 스타트에서만).
- 화면 회전 시 스테일 설치 확인 인텐트가 재생돼 죽은 설치 페이지를 다시 열던 것 차단.

### #6 "알림 오긴하는데 팝업으로 안옴" (원인 제거 + 확인 수단)

- 코드 경로에서 헤드업을 막는 결함은 발견되지 않았다(채널 HIGH·순서·쿨다운·빌더 전수 확인). 유력 원인은 **재시작·재연결 스윕이 최근 6시간치 수신함을 한꺼번에 HIGH 채널 배너로 터뜨리는 버스트** — One UI 적응형 알림이 채널을 무음으로 강등시키는 전형적 입력이고, 강등은 사용자 잠금이라 코드로 못 되돌린다.
- **스윕 메시지는 별도 저중요도 채널("놓친 메시지")로 조용히 게시**한다. 배너는 라이브 수신만.
- 강등됐을 수 있는 v2 채널을 폐기하고 **v3 채널로 재시작**한다(버스트 입력이 사라졌으니 재강등 없음).
- 분할 화면처럼 "실행 중이지만 앞에 없는" 상태에서 열린 대화의 알림이 통째로 삼켜지던 것 수정(onStart/onStop → onResume/onPause).
- 죽은 `setGroup` 제거(그룹당 자식 1개·요약 없음 — One UI 번들링 변수만 만들던 코드).
- 설정에 **"알림 팝업" 카드** 추가 — 시스템 채널 설정을 바로 열어 중요도가 '높음'인지, One UI '알림 팝업 스타일'이 '간단히'가 아닌지 확인할 수 있다. 여기서부터는 기기 설정 영역이다.

- Android versionCode 25 / versionName 0.12.2. 단위 테스트 200 통과, lint·debug APK 빌드 통과.

## v0.12.1 (2026-08-24) — 알림·MMS 한글 깨짐·대화에서 번호 차단

### 알림

- **채널을 새로 만들었다**(`securemsg_sms` → `securemsg_sms_v2`, IMPORTANCE_HIGH + 진동). 안드로이드는 기존 채널의 중요도 변경을 무시하므로 ID 교체가 유일한 방법이고, 옛 채널은 삭제한다. 이제 헤드업 배너가 뜬다.
- **알림이 메시지 단위 → 대화 단위.** 이전에는 메시지마다 별도 알림이 쌓여 패키지 활성 알림 상한(50)에 닿으면 `notify()`가 조용히 무시됐다 — 알림이 아예 끊기는 원인.
- **대화를 열면 알림이 사라진다.** 이전에는 알림을 눌러 들어온 경우에만 정리됐고, 앱을 직접 열어 읽으면 계속 남았다.
- **브리지 임포트 경로에 알림이 없던 것을 추가.** 리시버가 브로드캐스트 시간 안에 끝내지 못하고 죽으면 그 문자는 저장은 되고 알림만 영구히 유실됐다.
- **6시간 게이트를 스윕에만 적용.** 라이브 수신(SMS_DELIVER 복구, content-location 없는 WAP push, 다운로드 실패 폴백)은 캐리어 타임스탬프가 몇 시간 전이어도 알린다. 밤새 권외였다가 아침에 받은 문자가 조용히 들어오던 문제.
- 지연된 MMS 재시도를 평생 1회 → 3회 백오프(15/60/240초)로. 파트 다운로드가 늦으면 영영 재시도되지 않았다.
- 화면에 열려 있는 대화로 온 메시지는 배너를 띄우지 않고, 연속 수신은 10초 쿨다운으로 묶는다.
- 상태바 아이콘을 단색 벡터로(기존은 대화상자용 컬러 드로어블), 제목은 전화번호 대신 연락처 이름, `MessagingStyle`·`CATEGORY_MESSAGE` 적용.
- 알림 권한을 설정 화면뿐 아니라 최초 실행·기본 SMS 앱 지정 직후에도 요청한다.

### 한글 깨짐

- **MMS 제목이 깨지던 것 수정.** 플랫폼은 제목을 ISO-8859-1 바이트 뷰로 저장하고 실제 인코딩은 `SUBJECT_CHARSET`에 넣는데, 이 컬럼을 읽지 않아 한글 제목이 그대로 깨져 알림·릴레이·차단 키워드 검사에 들어갔다. 제목에만 키워드가 있는 스팸이 차단되지 않던 원인이기도 하다.
- **MMS 첨부 텍스트 파트를 한 번도 못 읽던 URI 버그 수정**(`content://mms/<id>/part/<pid>` → `part/<pid>`). 이 경로가 살아나면서 선언된 charset → Content-Type charset → UTF-8 순으로 엄격 디코딩한다.
- **송신 MMS**: WSP 인용 옥텟이 `0x1B`로 잘못 나가던 것을 `0x7F`로, 텍스트 파트 Content-Type에 charset 파라미터 추가. 첫 바이트가 0x7F를 넘을 때(=한글로 시작할 때)만 터져서 "가끔"으로 보였다.
- 수신 SMS 본문 자체에서는 결함을 찾지 못했다. 억지 수정 대신 다음 재현 시 실제 charset이 남도록 진단 로그를 넣었다(본문은 기록하지 않음).

### 대화에서 번호 차단

- 대화 헤더의 ⋮ 메뉴에 **차단 / 차단 해제**. 설정의 발신번호 차단과 같은 정규화·같은 서버 동기화를 쓴다.
- 해제는 저장된 모든 규칙 형태를 지운다. 정규화된 값만 지우면 `010…` 형태의 옛 규칙이 남아 UI는 해제됐다고 하는데 실제로는 계속 차단되는 상태가 됐다.
- 서버 반영 실패 시 문구로 알린다(오프라인에서 조용히 실패하던 것).
- 확인 문구가 실제 동작을 말한다 — 대화는 목록에 남고 기존 메시지가 가려지며, 이 폰이 중계한 메시지가 대상이라 내가 보낸 문자도 함께 가려진다.
- 헤더에 차단 상태(`· 차단됨`)를 표시한다.

- Android versionCode 24 / versionName 0.12.1. 단위 테스트 200 통과, lint·debug APK 빌드 통과.

## v0.12.0 (2026-08-23) — 폰 화면 재설계

Android를 "설정 도구"에서 "문자앱"으로. 네 장의 시안(대화 목록·연락처·대화·설정)을 그대로 구현했다.

- **하단 내비 메시지 · 연락처 · 설정.** "메시지 | 차단·설정" 탭 제거. 상태 칩은 헤더 우상단으로, 새 문자는 FAB로.
- **연락처 탭 신설.** 폰 주소록을 이 기기에서 읽어 초성 인덱스로 표시(ㄱㄴㄷ…, 그 외 첫 글자). 대화가 있는 사람은 바로 열리고, 없는 사람은 번호 입력 없이 새 대화로. 맨 위 "번호로 새 문자".
- **즐겨찾기.** 연락처를 길게 누르면 별. 맨 위 섹션에 모이고 아바타에 별 배지. 이 기기에만 저장(서버 미전송).
- **대화 목록 행**에 마지막 메시지·시각·안 읽음 개수. 안 읽음 행은 카드로 떠오른다. 개수는 마지막으로 연 시각 이후 수신 메시지 수(로컬 prefs + `countIncomingSince`).
- **대화 화면**: 뒤로·아바타·이름·번호 헤더가 상태바까지 이어지고, 말풍선 안에 시각·통신사 상태, 날짜 구분 알약, 차단 메시지는 접힌 칩. 컴포저는 알약 입력 + 원형 전송.
- **설정**: 기기 보안이 맨 위. 승인 요청은 카드 안 인셋으로, "QR 스캔으로 승인"만 주 버튼(번호 비교 없는 v1 승인은 고스트 버튼으로 명시). 격리된 스팸·발신번호 차단·연락처 이름 동기화·앱 업데이트는 행 목록.
- **회귀 수정(검토에서 발견)**: 발송 실패 문구가 대화 화면에서 안 보이던 것 → 컴포저 위 알림 줄, 번호 컴포저에서 발송 실패 시 사용자가 고립되던 것 → 생성된 스레드로 이동, 대화 열림 상태가 한 프레임 늦어 화면이 튀던 것 → 상태 호이스팅, 탭 전환마다 주소록 재조회·검색어 초기화 → 상태 호이스팅, 로그아웃·기기 초기화 시 즐겨찾기·마지막 열람 prefs 미삭제 → 정리 추가.
- 공용 컴포넌트 `SmBottomNav`·`SmSearchPill`·`SmFab`·`SmConversationRow`·`SmContactRow`·`SmChatHeader`·`SmComposer` 등을 `Theme.kt`에 추가. 아이콘은 material-icons-core + Canvas 경로, 새 의존성 없음. `Theme.kt` 밖 색 리터럴 0.
- Android versionCode 23 / versionName 0.12.0. 단위 테스트 168 통과, lint·debug APK 빌드 통과.
