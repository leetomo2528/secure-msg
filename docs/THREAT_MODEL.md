...
이메일·전화번호·복구 코드·관리자 재설정을 포함한 비밀번호 복구 수단을 제공하지 않는다. 비밀번호는 메시지 암호화 키가 아니므로 분실 즉시 서버 데이터나 기존 기기의 로컬 메시지가 삭제되지는 않는다. 현재 로그인된 세션은 만료 전까지 동작할 수 있지만, 이는 공식 복구 수단이 아니며 로그아웃·기기 폐기·토큰 만료 후에는 비밀번호 없이 토큰을 다시 발급받거나 새 기기를 등록할 수 없다.
...
- [ ] QR 기반 기존 기기 간 과거 내역 전송 또는 복구키 기반 암호화 백업
- [ ] 다중 서버·백업·감사 로그
